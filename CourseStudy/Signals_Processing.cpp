...
double Signals_Processing::Correlation_omp_jtids_with_phd_filtering(int delay_size, const vector<complex<double>>& ImSignal1, \
	const  vector<complex<double>>& ImSignal2, \
	vector <double>& ResearchRrr, int& found_delay, int& delay_lama, int win_size)
{
	//////////////////// Подготовка входных сигналов
...
#pragma omp parallel for
	for (int i = 0; i < this->operating_frequencies.size(); i++)
	{
		this->phd_filtering(this->FHSS_Signals_initial_fl[i], win_size);
		this->phd_filtering(this->FHSS_Signals_fl[i], win_size);
	}
	ResearchRrr.clear();
	ResearchRrr.resize(this->FHSS_Signals_initial_fl[0].size());
	for (int i = 0; i < this->operating_frequencies.size(); i++)
